...
func main() {
	...
	vmWatcher := watcher.NewExternalWatcher(&proxmox.VirtualMachineFetcher{Client: mgr.GetClient()},
		append(watcherOpts,
			...
			watcher.WithAutoRegister(mgr.GetCache(), &proxmoxv1alpha1.VirtualMachine{}, proxmox.VMConfigExtractor, readinessRetry),
	...
	managedVMWatcher := watcher.NewExternalWatcher(&proxmox.ManagedVirtualMachineFetcher{Client: mgr.GetClient()},
		append(watcherOpts,
			...
			watcher.WithAutoRegister(mgr.GetCache(), &proxmoxv1alpha1.ManagedVirtualMachine{}, proxmox.ManagedVMConfigExtractor, readinessRetry),
			watcher.WithAutoRegisterFilter(generationFilter),
		)...)
	containerWatcher := watcher.NewExternalWatcher(&proxmox.ContainerFetcher{Client: mgr.GetClient()},
		append(watcherOpts,
			watcher.WithMetrics("Container"),
			watcher.WithAutoRegister(mgr.GetCache(), &proxmoxv1alpha1.Container{}, proxmox.ContainerConfigExtractor, readinessRetry),
			watcher.WithAutoRegisterFilter(generationFilter),
		)...)
